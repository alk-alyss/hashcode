...
files = ["a_an_example.in.txt", "b_better_start_small.in.txt", "c_collaboration.in.txt", "d_dense_schedule.in.txt",
         "e_exceptional_skills.in.txt", "f_find_great_mentors.in.txt"]
if args.file is None:
    args.file = "f"
filename = files[ord(args.file) - 97]
...
try:
    ...
    print(f"day: {day}")
    print(f"score: {score}")
    print(f"nonScoringProjects: {nonScoringProjects}")
    print(f"len(done): {len(done)}")
